...
function renderReminders() {
    ...
    remindersList.innerHTML = userReminders.map(reminder => {
        const daysUntilText = getDaysUntil(reminder.date);
        
        // Calculate actual days for urgency logic
        ...
        // Color logic: red if ≤7 days, yellow if ≤14 days, green if >14 days
        let urgencyClass = '';
        if (diffDays <= 7) {
            urgencyClass = 'urgent'; // Red - very soon!
        } else if (diffDays <= 14) {
            urgencyClass = 'soon'; // Yellow - coming up
        } else {
            urgencyClass = 'distant'; // Green - plenty of time
        }
        
        // Check if gift purchased
        const isPurchased = reminder.giftPurchased || false;
        
        return `
            <div class="reminder-item ${isPurchased ? 'purchased' : urgencyClass}">
                <div class="reminder-info">
                    <h3>${reminder.name} ${isPurchased ? '<span class="purchased-badge">✓ SORTED</span>' : ''}</h3>
                    <p>${capitalizeFirst(reminder.occasion)} • ${formatDate(reminder.date)}</p>
                    ${isPurchased ? 
                        '<span class="reminder-countdown purchased">🎁 Gift purchased</span>' :
                        `<span class="reminder-countdown ${urgencyClass}">${daysUntilText}</span>`
                    }
                    <label class="gift-purchased-checkbox">
                        <input 
                            type="checkbox" 
                            ${isPurchased ? 'checked' : ''} 
                            onchange="toggleGiftPurchased('${reminder.id}', this.checked)"
                        >
                        <span>Gift purchased</span>
                    </label>
                </div>
                <div class="reminder-actions">
                    <button onclick="shopGifts('${reminder.occasion}')" ${isPurchased ? 'disabled' : ''}>Shop Gifts</button>
...
// Toggle Gift Purchased
async function toggleGiftPurchased(reminderId, isPurchased) {
    try {
        await firebase.firestore()
            .collection('users')
            .doc(currentUser.uid)
            ...
            .doc(reminderId)
            .set({
                giftPurchased: isPurchased,
                giftPurchasedDate: isPurchased ? firebase.firestore.FieldValue.serverTimestamp() : null
            }, { merge: true }); // Use merge to add fields without overwriting existing ones
        ...
        // Show confirmation
        if (isPurchased) {
            console.log('Gift marked as purchased!');
        }
    } catch (error) {
        console.error('Error updating gift purchased status:', error);
        ...
        await fetch('/.netlify/functions/mailchimp-sync-reminders', {
            ...
            body: JSON.stringify({
                ...
                reminders: userReminders.map(r => ({
                    ...
                    occasion: r.occasion,
                    date: r.date,
                    giftPurchased: r.giftPurchased || false
                })),
            }),
        });
    } catch (error) {
...
window.toggleGiftPurchased = toggleGiftPurchased;
